...
INTEGER Label;
END;
train_data_tmp := DATASET('~thesis::ii::bigdata::artrain', value_record, CSV);
test_data_tmp := DATASET('~thesis::ii::bigdata::artest', value_record, CSV);
//Train
...
//Naive Bayes
learner3 := ML.Classify.NaiveBayes;
nbModel := learner3.LearnC(indepTrainDataC, trainlabel);  //Naive Bayes Model 
//Classification Phase
results3 := learner3.ClassifyC(indepTestDataC,nbModel);
//Comparing to Original
compare3 := ML.Classify.Compare(testlabel, results3);
OUTPUT(compare3.CrossAssignments,ALL);  //CrossAssignment results

//Random Forest
treeNum := 25;
fsNum := 3;
Pure := 1.0;
maxLevel := 5;  //Random Forest parameters
...
//Learning Phase
MinNumObj := 2;
//MaxLevel := 5;   //Decision Tree parameters
