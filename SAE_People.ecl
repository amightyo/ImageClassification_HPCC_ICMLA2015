...
INTEGER4 hl := 32; //128;//number of nodes in the hiddenlayer  32: Total Cluster Time: 3:50.118
INTEGER4 f := 416;//number of input features 16x16 image patches
...
input_data_tmp := DATASET('~thesis::ii::bigdata::people_reye_1000samples_x16_y26_normalizednorm', value_record, CSV); 

ML.AppendID(input_data_tmp, id, input_data);
OUTPUT(input_data(id < 1000), NAMED('input_data'));
//(id < 1000)
...
indepDataC := normalizeData(indepDataC);
*/
...
REAL8 sparsityParam  := 0.05; //0.1;  0.15  //The desired average activation for the hidden units
REAL8 BETA := 0.9; //0.5; //0.1;  //weight of sparsity penalty term
REAL8 ALPHA := 0.1;   //learning rate
REAL8 LAMBDA := 0.004; //0.0001; //0.1;  //weight decay term
...
OUTPUT(Extractedweights(no=2), named ('Extractedweights'));
OUTPUT(Extractedweights(no=2),, '~online::ii::bigdata::extractedsaeweightmouth', OVERWRITE, CSV);
...
//Converting the weight matrix to Dataset 


//Output the dataset for despray